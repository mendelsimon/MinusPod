...
| Mark | Left in the audio, but detected and published as a chapter so a chapter-aware player can skip it. Unlike Keep, a marked segment travels the whole cut path first, so the validator, the reviewer, hold rules and the min-cut-confidence gate all still apply to it; it diverges only at the audio edit. Use it when something downstream will act on the marker, where a false positive costs you content just as a cut would. Requires `AD_CHAPTERS_ENABLED`. Each marked span is published as a chapter titled by `AD_CHAPTER_TITLE_FORMAT` (default `[mp:{category}]`), so a filtering client can act per category rather than skipping every marker alike. |
